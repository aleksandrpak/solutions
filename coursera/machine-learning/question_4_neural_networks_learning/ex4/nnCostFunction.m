...
for i = 1:m
    ...
    hi = sigmoid(a2 * Theta2');
    di3 = zeros(1, num_labels);
    ...
    for k = 1:num_labels
        ...
        J = J + (-yi * log(hik) - (1 - yi) * log(1 - hik));
    end

    di2 = di3 * Theta2(:, 2:end) .* sigmoidGradient(z2);

    Theta1_grad = Theta1_grad + (xi' * di2)';
    Theta2_grad = Theta2_grad + (a2' * di3)';
end
